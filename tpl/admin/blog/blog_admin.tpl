...
<ul class="nav nav-tabs">
    <li role="presentation">
        <a class="nav-link {if $status eq '1'}active{/if}"
           href="http://{$WEB_ROOT}{$PUBLIC_DIR}/admin/blog/?status=1&amp;page=1">
            Drafts
        </a>
    </li>
    <li role="presentation">
        <a class="nav-link {if $status eq '2'}active{/if}"
           href="http://{$WEB_ROOT}{$PUBLIC_DIR}/admin/blog/?status=2&amp;page=1">
            Published
        </a>
    </li>
    {if $super eq 1}
        <li role="presentation">
            <a class="nav-link {if $status eq '3'}active{/if}"
               href="http://{$WEB_ROOT}{$PUBLIC_DIR}/admin/blog/?status=3&amp;page=1">
                Deleted
            </a>
        </li>
    {/if}
    ...
    <table class="table">
        ...
        {section name=blog_item loop=$blogs}
            <tr class="{cycle values="alt," }">
                ...
                <td>
                    <a href="http://{$WEB_ROOT}{$PUBLIC_DIR}/admin/blog/view/{$blogs[blog_item].id}">{$blogs[blog_item].heading}</a>
                </td>
